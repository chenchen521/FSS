...
from FCN.FCN_layers import conv_layer, pool_layer, dropout_layer, deconv_layer
...
def inference(image, num_of_class, weights, keep_prob):
    ...
    with tf.variable_scope('inference'):
        ...
        pool1 = pool_layer('pool1', conv1_2, 2, 2, 2, 2, 'SAME', tf.nn.avg_pool)  #(112,112,64)
        ...
        pool2 = pool_layer('pool2', conv2_2, 2, 2, 2, 2, 'SAME', tf.nn.avg_pool)  #(56,56,128)
        ...
        pool3 = pool_layer('pool3', conv3_4, 2, 2, 2, 2, 'SAME', tf.nn.avg_pool)  #(28,28,256)
        ...
        pool4 = pool_layer('pool4', conv4_4, 2, 2, 2, 2, 'SAME', tf.nn.avg_pool) #(14,14,512)
        ...
        pool5 = pool_layer('pool5', conv5_4, 2, 2, 2, 2, 'SAME', tf.nn.max_pool) #(7,7,512)
        ...
        # Conv7
        conv7 = conv_layer('conv7', drop6, 1, 1, 4096, 4096, 1, 1, 'SAME', tf.nn.relu) #(7,7,4096)
 
        drop7 = dropout_layer('drop7', conv7, keep_prob)

        # Conv8
        conv8 = conv_layer('conv8', drop7, 1, 1, 4096, num_of_class, 1, 1, 'SAME', tf.identity) #(7,7,2)

        # upsampling inference
        # Deconv1 -  32x
        deconv1_shape = tf.shape(pool4)
 
        deconv1_out_channels = pool4.get_shape()[3].value  #512

        deconv1 = deconv_layer('deconv1', conv8, 4, 4, deconv1_out_channels, num_of_class,
                               deconv1_shape, 2, 2, 'SAME') 

        fuse1 = tf.add(deconv1, pool4, name='fuse1')  #(14,14,512)

        # Deconv2 - 16x
        deconv2_shape = tf.shape(pool3) # (28,28,256)

        deconv2_out_channels = pool3.get_shape()[3].value  #256

        deconv2 = deconv_layer('deconv2', fuse1, 4, 4, deconv2_out_channels, deconv1_out_channels,
                               deconv2_shape, 2, 2, 'SAME')

        fuse2 = tf.add(deconv2, pool3, name='fuse2')  #(28,28,256)

        # Deconv3 - 8x
        deconv3_shape = tf.shape(image)   #(2,224,224,3)
        deconv3_shape = tf.stack([deconv3_shape[0], deconv3_shape[1], deconv3_shape[2], num_of_class]) #(224,224,3,2)
        deconv3 = deconv_layer('deconv3', fuse2, 16, 16, num_of_class, deconv2_out_channels,
                               deconv3_shape, 8, 8, 'SAME')      #(2,224,224,2)

        prediction = tf.expand_dims(tf.argmax(deconv3, dimension=3, name='pred_annotations'), dim=3) #(2,224,224,1)
...
def loss(logits, annotation):
    with tf.name_scope('loss'):
        ...
        labels = tf.squeeze(annotation, squeeze_dims=[3])  #labels (2,224,224,1)  logits (2,224,224,2)
